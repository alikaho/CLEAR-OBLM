...
function [beam_loss_dist_fiber] = Find_fiber_loss_dist_upstream_eff_refr_idx(loss_idx_up)
    ...
    syms f(x)
    f(x) = (x + poly_coeffs(1) * x^4 + poly_coeffs(2) * x^3 + poly_coeffs(3) * x^2 + poly_coeffs(4) * x) - (light_speed * loss_idx_up * 1e-9);
