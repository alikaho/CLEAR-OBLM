...
% plot_rise_times_less(smooth_up_data,smooth_down_data);
...
function [up_data, down_data, smooth_up_data, smooth_down_data, screens] = get_data(table)
    ...
    screens_unsorted = lookup{:, 'Screen'};27072025-" + char(timestamps(i)) + ".txt", "BTV screen data/BLM_GUI_data_16072025_BTV_" + screens_unsorted(i) + ".txt")
    end
    
    % copy jpg files
    for i = 1:length(timestamps)
        copyfile("Raw BLM data/BLM_GUI_data_16072025-" + char(timestamps(i)) + ".jpg", "Corrector magnet data/BLM_GUI_data_16072025_magnet_" + magnets(i) + ".jpg")
    end
...
function plot_rise_times_less(smooth_up_data, smooth_down_data)
    ...
    rise_indices_up(1) = Find_rise_time_CFD(smooth_up_data(3, :));
    rise_indices_up(2) = Find_rise_time_CFD(smooth_up_data(5, :));
    rise_indices_up(3) = Find_rise_time_CFD(smooth_up_data(6, :));
    rise_indices_up(4) = Find_rise_time_CFD(smooth_up_data(7, :));

    rise_indices_down(1) = Find_rise_time_CFD(smooth_down_data(3, :));
    rise_indices_down(2) = Find_rise_time_CFD(smooth_down_data(5, :));
    rise_indices_down(3) = Find_rise_time_CFD(smooth_down_data(6, :));
    rise_indices_down(4) = Find_rise_time_CFD(smooth_down_data(7, :));

    reconstructed_positions = Find_beam_loss_dist(rise_indices_up, rise_indices_down);
    ...
    text(26, 26, [' Fit: y = ' num2str(gradient) 'x + ' num2str(offset)])
    ...
    text(26, 25, ['RMS value = ' num2str(rms)])
